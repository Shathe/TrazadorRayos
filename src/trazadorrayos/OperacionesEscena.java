		...
		if (figura != null && Interseccion.intersecta(rayo, figura) != null) {
			...
			if (refractadoParaSalir) {
				...
				if (noVisible == 1) {
					...
					// ambiental
					double intensidadR = escena.getFoco()
							.getIntensidadAmbiente() * figura.kd.getRed();
					double intensidadG = escena.getFoco()
							.getIntensidadAmbiente() * figura.kd.getGreen();
					double intensidadB = escena.getFoco()
							.getIntensidadAmbiente() * figura.kd.getBlue();
					// difusa
					...
					// 2*(L*N)N
					reflejado2.scale(aux2);
					// 2*(L*N)N - L
					reflejado2.sub(rayoAlFoco, reflejado2);
